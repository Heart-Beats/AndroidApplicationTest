 ...
 */
class CornerLayout @JvmOverloads constructor(context: Context?, attrs: AttributeSet? = null) : ViewGroup(context, attrs) {
    ...
    override fun onLayout(changed: Boolean, l: Int, t: Int, r: Int, b: Int) {
        if (changed) {
            ...
            val layoutParams = first.getLayoutParams() as CornerLayoutParams
            val left: Int
            val top: Int
            when (layoutParams.mCorner) {
                ...
                CornerLayoutParams.CORNER_RIGHT_TOP -> {
                    top = paddingTop + layoutParams.topMargin
                    left = width - paddingRight - first.getMeasuredWidth() - layoutParams.rightMargin
                }
                CornerLayoutParams.CORNER_LEFT_BOTTOM -> {
                    top = height - paddingBottom - first.getMeasuredHeight() - layoutParams.bottomMargin
                    left = paddingLeft + layoutParams.leftMargin
                }
                CornerLayoutParams.CORNER_RIGHT_BOTTOM -> {
                    top = height - paddingBottom - layoutParams.bottomMargin - first.getMeasuredHeight()
                    left = width - paddingRight - layoutParams.rightMargin - first.getMeasuredWidth()
                }
                else -> {
                    left = paddingLeft + layoutParams.leftMargin
                    top = paddingTop + layoutParams.topMargin
    ...
    override fun generateLayoutParams(p: LayoutParams?): LayoutParams {
        return if (p is MarginLayoutParams) {
            CornerLayoutParams(p as MarginLayoutParams?)
        } else CornerLayoutParams(p)
    }
    ...
    class CornerLayoutParams : MarginLayoutParams {
        ...
        constructor(c: Context, attrs: AttributeSet?) : super(c, attrs) {
            val a = c.obtainStyledAttributes(attrs, R.styleable.CornerLayout_Layout)
            mCorner = a.getInt(R.styleable.CornerLayout_Layout_corner, CORNER_LEFT_TOP)
            a.recycle()
        }
